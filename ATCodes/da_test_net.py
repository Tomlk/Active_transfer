...
def start_test(ratio,epoch_index,s_t_ratio,dataset_name,GPUID,select_strategy, types,source_list=None,target_list=None):
    ...
    result=TEST.excute(_GPUID=GPUID,_cuda=True,_gc=True,_lc=True,_part=part,_dataset=dataset,_model_dir=model_dir,_output_dir=output_dir,
                       _modelepoch=modelepoch,_ratio=ratio,_epochindex=epoch_index,_st_ratio=s_t_ratio,_test_flag=False,_select_strategy=select_strategy, _types=types,_source_list=source_list,_target_list=target_list)
