...
import os
from active_tools import filetools_copy
...
def random_sample(imgs_path):
    ...
    for i in range(len(imgslist)):
        imgslist[i]=os.path.join(imgs_path,imgslist[i])
    ...
    # imgs = filetools_copy.get_all_file_list(imgs_path)
    # xmls = filetools_copy.get_all_file_list(xmls_path)
    ...
    # #     return
    # # 产生一个有 M 个不重复的随机数的列表
    # random_list = filetools_copy.random_list_generator(0, len(imgs), len(imgs))
    # # 用随机数抽取文件
...
def uncertain_sample(detections_for_all_images, M, lambda_=3):
    """
    ...
    # 存储本轮采样的图片
    # import time
    # filetools_copy.text_save('pre_5_percent_images_' + str(time.time()).split('.')[0] + '.txt', pre_5_percent_images)
