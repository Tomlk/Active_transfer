#!/bin/bash
dataset="sim10k"
save_dir="./data/experiments/SW_Faster_ICR_CCR/sim10k/model"
...
# #sim10k 域自适应
# python da_train_origin.py --max_epochs 20 --cuda --dataset ${dataset} --net ${net} --save_dir ${save_dir} --pretrained_path ${pretrained_path} --checkpoint_interval ${checkpoint_interval} --gc --lc --da_use_contex  --gpu_id ${GPUID}


#sim10k at random 0
python da_train_net.py --cuda --dataset ${dataset} --net ${net} --save_dir ${save_dir} --pretrained_path ${pretrained_path} --checkpoint_interval ${checkpoint_interval} --gc --lc --da_use_contex --round_num ${round_num} --gpu_id ${GPUID} --st_ratio ${st_ratio} --select_strategy 0 --first_not_transfer 0
